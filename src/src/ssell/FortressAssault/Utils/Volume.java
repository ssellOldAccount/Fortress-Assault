...
public class Volume {
	...
	private final FortressAssault war;	

	public Volume(String name, FortressAssault war, World world) {
		this.name = name;
		this.war = war;
		this.world = world;
	}
...
//		return 0;
		...
		try {
					...
					for(int j = 0; j < getSizeY(); j++){
						...
						for(int k = 0;k < getSizeZ(); k++) {
							try {
								...
								this.getWar().logWarn("Failed to save block in volume " + getName() + ". Saved blocks so far:" + noOfSavedBlocks 
			...
			this.getWar().logWarn("Failed to save volume " + getName() + " blocks. Saved blocks:" + noOfSavedBlocks 
					+ ". Error at x:" + x + " y:" + y + " z:" + z + ". Exception:" + e.getClass().toString() + " "+ e.getMessage());
			e.printStackTrace();
		}
					...
					for(int j = 0; j < getSizeY(); j++){
						...
						for(int k = 0;k < getSizeZ(); k++) {
							try {
								...
								this.getWar().logWarn("Failed to reset block in volume " + getName() + ". Visited blocks so far:" + visitedBlocks 
